...
function generateRuleBasedSuggestions(summary: FinancialSummary): GoalSuggestion[] {
  ...
  // Cash flow improvement goal
  if (summary.net_cash_flow < summary.monthly_income * 0.2 && summary.monthly_income > 0) {
    const spendingTarget = Math.round(summary.monthly_income * 0.8);
    const targetDate = new Date(now);
    targetDate.setMonth(targetDate.getMonth() + 3);
    suggestions.push({
      goal_type: 'spending_limit',
      name: 'Monthly Budget',
      suggested_target: spendingTarget,
      suggested_date: targetDate.toISOString().split('T')[0],
      reasoning: `Limit monthly spending to $${spendingTarget.toLocaleString()} to keep at least 20% of income as surplus cash flow.`,
      linked_account_ids: savingsAccounts.map(a => a.account_id),
    });
  }
