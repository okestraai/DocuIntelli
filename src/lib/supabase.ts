...
// Detect impersonation mode: when the app opens in an impersonation tab,
// use sessionStorage instead of localStorage so the impersonated session
// is isolated to this tab and doesn't overwrite the admin's session.
const isImpersonationTab =
  new URLSearchParams(window.location.search).has('impersonate_token') ||
  sessionStorage.getItem('docuintelli_impersonated') === 'true';

export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
    flowType: 'pkce',
    detectSessionInUrl: !isImpersonationTab, // Don't auto-detect URL tokens in impersonation tabs
    autoRefreshToken: true,
    persistSession: true,
    ...(isImpersonationTab && {
      storage: sessionStorage, // Isolate session to this tab only
    }),
  },
})
