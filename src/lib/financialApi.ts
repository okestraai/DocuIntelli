...
export async function getConnectedAccounts(): Promise<{ accounts: any[]; account_count: number }> {
  const session = await getSession();
  ...
  const data = await res.json();
  return { accounts: data.accounts || [], account_count: data.account_count || 0 };
}
...
export async function getDetectedLoans(): Promise<DetectedLoanPrompt[]> {
  const session = await getSession();
  const res = await fetch(`${API_BASE}/api/financial/detected-loans`, {
    headers: backendHeaders(session.access_token),
  });
...
export async function getAnalyzedLoans(): Promise<AnalyzedLoan[]> {
  const session = await getSession();
  const res = await fetch(`${API_BASE}/api/financial/analyzed-loans`, {
    headers: backendHeaders(session.access_token),
  });
...
export async function getLoanAnalysis(detectedLoanId: string): Promise<LoanAnalysis | null> {
  const session = await getSession();
  const res = await fetch(`${API_BASE}/api/financial/loan-analysis/${detectedLoanId}`, {
    headers: backendHeaders(session.access_token),
  });
