                ...
                {navItems.map(({ id, label, icon: Icon }) => (
                  <button
                    ...
                    className={`group relative flex items-center justify-center p-3 rounded-xl transition-all duration-200 ${
                      currentPage === id
                        ? id === 'admin' ? 'bg-red-50 text-red-700 shadow-sm' : 'bg-emerald-50 text-emerald-700 shadow-sm'
                        : id === 'admin' ? 'text-slate-600 hover:text-red-700 hover:bg-red-50' : 'text-slate-600 hover:text-emerald-700 hover:bg-slate-100'
                    }`}
                  >
