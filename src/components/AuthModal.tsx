...
import { X, ShieldCheck, Mail, Lock, Eye, EyeOff, KeyRound } from 'lucide-react';
import { signUp, signIn, resetPasswordWithOTP, verifyOTP, resendOTP } from '../lib/supabase';
...
type AuthStep = 'auth' | 'verify-signup' | 'verify-reset';
...
export function AuthModal({ onClose, onAuth }: AuthModalProps) {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      const type = authStep === 'verify-signup' ? 'signup' : 'recovery';
      ...
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl max-w-md w-full p-6 sm:p-8 relative shadow-2xl">
          ...
          <form onSubmit={handleVerifyOTP} className="space-y-4">
            <div>
              <label htmlFor="otp" className="block text-xs sm:text-sm font-medium text-slate-700 mb-2">
                Verification Code
              </label>
          ...
          <div className="mt-4 sm:mt-6 text-center space-y-2">
            ...
            <button
              ...
              disabled={isLoading}
            >
              Back to {authStep === 'verify-signup' ? 'signup' : 'password reset'}
            </button>
          </div>
        </div>
      </div>
