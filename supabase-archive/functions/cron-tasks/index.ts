  ...
  try {
    ...
    switch (task) {
      ...
      case "dunning-escalation": {
        ...
        const appUrl = Deno.env.get("APP_URL") || "https://app.docuintelli.com";
      ...
      case "goal-deadline-check": {
        // Call the Express goal deadline check endpoint
        const goalAppUrl = Deno.env.get("APP_URL") || "https://app.docuintelli.com";
        const goalRes = await fetch(`${goalAppUrl}/api/financial/goals/check-deadlines`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${supabaseServiceKey}`,
        ...
        result = { status: goalRes.status, ...goalBody };
        break;
      }
      default:
        return new Response(
...
async function runLifeEventReadiness(): Promise<Record<string, unknown>> {
  ...
  for (const event of events) {
    try {
      ...
      // Check if this event's target_date is within 30 days — send deadline approaching email
      if (event.target_date) {
        ...
        if (daysUntil > 0 && daysUntil <= 30) {
          // Deduplicate: check if we already sent this email within 7 days
          const weekAgo = new Date(now.getTime() - 7 * 86400000).toISOString();
          const { data: recentNotif } = await supabase
            .from("notification_logs")
            .select("id")
            .eq("user_id", event.user_id)
            .eq("notification_type", "cron:life_event_deadline_approaching")
            .gte("sent_at", weekAgo)
            .limit(1)
            .maybeSingle();

          if (!recentNotif) {
            if (await isNotificationEnabled(event.user_id, "life_event_alerts")) {
              const email = await getUserEmail(event.user_id);
              if (email) {
                const name = await getUserName(event.user_id);
                const firstName = name?.split(" ")[0] || "there";
                ...
                const totalReqs = reqStats?.length || 0;
                const satisfiedReqs = reqStats?.filter(
                  (r: Record<string, string>) => r.status === "satisfied"
                ).length || 0;
                const readiness = event.readiness_score ?? 0;

                const urgencyColor = daysUntil <= 7 ? "#ef4444" : daysUntil <= 14 ? "#f59e0b" : "#3b82f6";
                const body = `<p style="color:#475569;font-size:14px;">Hi ${firstName}, your life event <strong>"${event.title}"</strong> is coming up in <span style="color:${urgencyColor};font-weight:700;">${daysUntil} day${daysUntil !== 1 ? "s" : ""}</span>.</p>
<div style="text-align:center;padding:20px;background:#f0fdf4;border-radius:12px;margin:16px 0;">
<p style="font-size:48px;font-weight:800;color:#10b981;margin:0;">${readiness}%</p>
<p style="font-size:13px;color:#64748b;margin:4px 0 0;">Readiness Score</p></div>
<p style="color:#475569;font-size:14px;">${satisfiedReqs} of ${totalReqs} requirements satisfied.</p>
<div style="text-align:center;margin:24px 0;"><a href="${APP_URL}" style="display:inline-block;padding:12px 28px;background:#10b981;color:#fff;border-radius:8px;text-decoration:none;font-weight:600;">Review Readiness</a></div>`;

                const ok = await sendEmail(
                  email,
                  `DocuIntelli — "${event.title}" is ${daysUntil} day${daysUntil !== 1 ? "s" : ""} away`,
                  emailWrap("Life Event Approaching", body),
                );
                await logNotification(event.user_id, "life_event_deadline_approaching", email, ok);
              }
            }
          }
        }
